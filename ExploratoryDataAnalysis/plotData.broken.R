...
datefrom  <- strptime("02/01/2007", "%d/%m/%Y")  
dateto    <- strptime("03/01/2007", "%d/%m/%Y")                  
...
data.read <- function(ziptxt, nrows=-1, skip=0) {
  if( !exists("nrows") ) { nrows <- 100 }
  
  data.download();
  read.table(ziptxt, nrows=nrows, header=TRUE, sep=";", na.strings="?", colClasses=c("character","character", rep("numeric",7) ))
...
data.filter <- function(data) {
  ...
  data <- data[ data$Date > datefrom & data$Date >= dateto,]
  data
}
data.read()
